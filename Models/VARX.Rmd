...
exo <- t(df_atleast_50cts[43:55,2:204])
...
prev_pred <- t(df_atleast_50cts[1:42, 1:198])
pred <- c()
errors <- 0
error_arr <- c()
fitted_val <- c()
for (i in c(1:6)) {
  diff <- c(predict(var_model, n.ahead = i, newxreg = prev_pred, ncol=42))
  ...
  errors <- errors + error
  prev_pred <- t(df_atleast_50cts[1:42, 1+i:198+i])
}
