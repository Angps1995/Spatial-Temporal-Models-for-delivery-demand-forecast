...
```{r}
# FIt GLM function 
fit_and_evaluate_glm = function(train, test, past_obs, past_mean, distr){
    num_locs = dim(train)[1]
    errors <- 0
    error_arr <- c()
    fitted_val <- c()
    for (i in c(1 : num_locs)) {  # go through all locations
        fit <- suppressWarnings(tsglm(as.numeric(train[i,]), model = list(past_obs = c(1 : past_obs), past_mean = past_mean), distr = distr)) # fit glm
...
result = fit_and_evaluate_glm(train_small_df, test_small_df, past_obs = 6, past_mean = 18, distr = 'poisson')
...
```{r}
# Fit and evaluate on smaller subset of dataset
result = fit_and_evaluate_glm(train_full_df, test_full_df, past_obs = 1, past_mean = 1, distr = 'poisson')
error_arr = result[1]
MSE = result[2]
