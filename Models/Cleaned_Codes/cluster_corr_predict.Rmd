---
title: "3 Step Approach, Clustering by correlation"
...
assign_clust_demand = function(clusters, clust_pred, train_df) {
    ...
    for (i in c(1:num_clust)) {
        ...
        clust_order_by_corr <- rowSums(train_df[clust_ind,])
        for (j in c(1:length(clust_ind))) {
            loc_prop <- clust_order_by_corr[j]/sum(clust_order_by_corr)
            for (t in c(1:6)) {
...
assign_cluster_by_corr = function(train, ori_df, num_clust) {
    "
    Cluster by correlation, aggregate the locations into clusters where the cluster demand is the summation of all its locations' demand
    Inputs:
      train_df: Training dataframe
    ...
    "
    km <- Kmeans(train, centers = num_clust, method = 'correlation')
    clusters <- list()
    df_cols <- list()
    for (i in c(1 : num_clust)) {
...
reassign_demand_to_loc_and_evaluate = function(cluster_pred, cluster_ind, train_df, ori_df) {
    ...
    for (i in c(1:6)) {
        ...
        errors <- errors + error
        prev_pred <- curr_pred
        for (j in c(1:num_locs)) {if (isFALSE(is.nan(scoring(act[j], curr_pred[j], distr=c("poisson"))['quadratic'][['quadratic']]))) {
          quad_score <- quad_score + scoring(act[j], curr_pred[j], distr=c("poisson"))['quadratic']}}
        #for (j in c(1:num_locs)) {quad_score <- quad_score + (-2*dpois(act[j],lambda=curr_pred[j]) + sapply(curr_pred[j], function(x){ sum(dpois(1:1000,lambda=act[j])^2) }))}
    }
    return (list("Errors_array" = error_arr, "Fitted_Values" = fitted_val, "MSE" = errors/6, 'Quadratic(Brier) Score' = quad_score/6))
}

get_clustering_by_corr_result = function(num_clust, ori_df, train_df, pred_method,p=6, struct="Basic", T1=156, T2=198) {
    "
    Main function to perform the 3 step approach using clustering by correlation
    Inputs:
      num_clust: Number of clusters
    ...
    "
    # Perform clustering
    ClusterResult <- assign_cluster_by_corr(train=train_df, ori_df=ori_df, num_clust = num_clust)
    ...
    print(paste0("MSE of clustering by correlation using ", num_clust, " clusters: ", MSE))
...
quad_score <- c()
for (i in clusters) {
    res <- get_clustering_by_corr_result(num_clust=i, ori_df = df_atleast_50cts_endo, train_df = train_small, pred_method='VAR')
    MSE <-c(MSE, res$MSE)
...
quad_score <- c()
for (i in clusters) {
    res <- get_clustering_by_corr_result(num_clust=i, ori_df = df_atleast_50cts_endo, train_df = train_small, pred_method='STAR')
    MSE <-c(MSE, res$MSE)
    quad_score <- c(quad_score, res$quad_score)
}
```
...
# Apply on full dataset using VAR (Try clustering groups 2-12)
...
quad_score <- c()
for (i in clusters) {
    res <- get_clustering_by_corr_result(num_clust=i, ori_df = full_df[1:839,], train_df = train_full, pred_method='VAR')
    MSE <-c(MSE, res$MSE)
    quad_score <- c(quad_score, res$quad_score)
}
plot(clusters, MSE, main="MSE against different number of clusters",cex.main=1.2, cex.lab=1.3)
lines(clusters, MSE)
dev.copy(jpeg,filename="/home/angps/Documents/Thesis/Report/Images/Cluster_Corr_MSE_plot.jpg");
dev.off ();
...
dev.copy(jpeg,filename="/home/angps/Documents/Thesis/Report/Images/Cluster_Corr_quad_plot.jpg");
dev.off ();
```


# Apply on full dataset using STAR (Try clustering groups 2-12)
...
quad_score <- c()
for (i in clusters) {
    res <- get_clustering_by_corr_result(num_clust=i, ori_df = full_df[1:839,], train_df = train_full, pred_method='STAR')
    MSE <-c(MSE, res$MSE)
    quad_score <- c(quad_score, res$quad_score)
}
plot(clusters, MSE, main="MSE against different number of clusters",cex.main=1.2, cex.lab=1.3)
lines(clusters, MSE)
dev.copy(jpeg,filename="/home/angps/Documents/Thesis/Report/Images/Cluster_Corr_STAR_MSE_plot.jpg");
dev.off ();

plot(clusters, quad_score, main="Quad Score against different number of clusters",cex.main=1.2, cex.lab=1.3)
lines(clusters,quad_score)
dev.copy(jpeg,filename="/home/angps/Documents/Thesis/Report/Images/Cluster_Corr_STAR_quad_plot.jpg");
dev.off ();
```

# Apply on full dataset using recovered data using VAR (Try clustering groups 2-12)
...
quad_score <- c()
for (i in clusters) {
    res <- get_clustering_by_corr_result(num_clust=i, ori_df = full_df[1:839,], train_df = rec_df, pred_method='VAR')
    MSE <-c(MSE, res$MSE)
    quad_score <- c(quad_score, res$quad_score)
}
plot(clusters, MSE, main="MSE against different number of clusters",cex.main=1.2, cex.lab=1.3)
lines(clusters, MSE)
dev.copy(jpeg,filename="/home/angps/Documents/Thesis/Report/Images/Cluster_rec_Corr_MSE_plot.jpg");
dev.off ();

plot(clusters, quad_score, main="Quad Score against different number of clusters",cex.main=1.2, cex.lab=1.3)
lines(clusters,quad_score)
dev.copy(jpeg,filename="/home/angps/Documents/Thesis/Report/Images/Cluster_rec_Corr_quad_plot.jpg");
dev.off ();
```


# Apply on full dataset using recovered data using STAR (Try clustering groups 2-12)
...
quad_score <- c()
for (i in clusters) {
    res <- get_clustering_by_corr_result(num_clust=i, ori_df = full_df[1:839,], train_df = rec_df, pred_method='STAR')
    MSE <-c(MSE, res$MSE)
    quad_score <- c(quad_score, res$quad_score)
}
plot(clusters, MSE, main="MSE against different number of clusters",cex.main=1.2, cex.lab=1.3)
lines(clusters, MSE)
dev.copy(jpeg,filename="/home/angps/Documents/Thesis/Report/Images/Cluster_rec_Corr_STAR_MSE_plot.jpg");
dev.off ();

plot(clusters, quad_score, main="Quad Score against different number of clusters",cex.main=1.2, cex.lab=1.3)
lines(clusters,quad_score)
dev.copy(jpeg,filename="/home/angps/Documents/Thesis/Report/Images/Cluster_rec_Corr_STAR_quad_plot.jpg");
dev.off ();
```