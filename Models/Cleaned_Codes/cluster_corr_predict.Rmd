...
assign_cluster_by_corr = function(train, ori_df, num_clust) {
    "
    Cluster by correlation, aggregate the locations into clusters by summing up all locations' demand in the respective cluster
    "
...
get_clustering_by_corr_result = function(num_clust, ori_df, train_df) {
...
```{r}
x<-get_clustering_by_corr_result(num_clust=3, ori_df = full_df[1:839,], train_df = train_full)
get_clustering_by_corr_result(num_clust=6, ori_df = full_df[1:839,], train_df = train_full)
get_clustering_by_corr_result(num_clust=9, ori_df = full_df[1:839,], train_df = train_full)
get_clustering_by_corr_result(num_clust=12, ori_df = full_df[1:839,], train_df = train_full)
```