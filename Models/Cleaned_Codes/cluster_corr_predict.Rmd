...
get_clustering_by_corr_result = function(num_clust, ori_df, train_df, p=6, struct="Basic", T1=156, T2=198) {
    "
...
```{r eval=FALSE}
get_clustering_by_corr_result(num_clust=3, ori_df = df_atleast_50cts_endo, train_df = train_small)
get_clustering_by_corr_result(num_clust=6, ori_df = df_atleast_50cts_endo, train_df = train_small)
get_clustering_by_corr_result(num_clust=9, ori_df = df_atleast_50cts_endo, train_df = train_small)
get_clustering_by_corr_result(num_clust=12, ori_df = df_atleast_50cts_endo, train_df = train_small)
```

# Apply on full dataset (Try clustering groups 1-12)
...
quad_score <- c()
for (i in clusters) {
    res <- get_clustering_by_corr_result(num_clust=i, ori_df = full_df[1:839,], train_df = train_full)
    MSE <-c(MSE, res$MSE)
