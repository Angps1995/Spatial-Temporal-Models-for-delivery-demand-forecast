...
get_clustering_by_loc_result = function(num_clust, loc_df, ori_df, train_df, p=6, struct="Basic", T1=156, T2=198) {
    "
...
quad_score <- c()
for (i in clusters) {
    res <- get_clustering_by_loc_result(num_clust=i, loc_df = mini_loc_df, ori_df = df_atleast_50cts_endo, train_df = train_small)
    MSE <-c(MSE, res$MSE)
...
quad_score <- c()
for (i in clusters) {
    res <- get_clustering_by_loc_result(num_clust=i, loc_df = loc_df, ori_df = full_df[1:839,], train_df = train_full)
    MSE <-c(MSE, res$MSE)
    quad_score <- c(quad_score, res$quad_score)
}
