...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth, height=0.7\textheight]{Images/cluster_corr_heatmap.jpg}
    \caption{Correlation heatmap for 9 clusters}
    \label{fig:Correlation heatmap for 9 clusters}
\end{figure}

\noindent We can observe from the sample correlation heatmap in Figure 14 that there exist strong correlation between certain clusters (like cluster 3 and 4 and cluster 6 and 8). To exploit the spatial correlation effects, VAR model was then trained on the clusters and is used to predict the total demand for each of the clusters. 
...
Our 3-step approach model display decent MSFE result. However, using VAR is still technically a linear model and future work could include using non-linear models like neural network, such as Long Short-Term Memory (LSTM) models or Convolutional LSTM models that also use spatial-temporal features to forecast the demand across locations. 
\newpage
\section{Appendix}
