...
\begin{document}
	...
	%\subtitle{(Classical methods for Spatio-Temporal Forecasting)}
...
\begin{figure}[H]
    ...
    \textbf{\caption{Demand across locations over time}}
...
\begin{figure}[H]
    ...
    \textbf{\caption{Scatter plot of mean counts against temperature}}
...
\begin{figure}[H]
    ...
    \textbf{\caption{Scatter plot of mean counts against pressure}}
...
\noindent As mentioned in section 3.2, we would first assess our model on the smaller dataset before testing it on the full dataset. Using the R package tscount from \cite{Tobias2017}, we fit each of the locations individually by a GLM model. For the parameters, we used the past 6 lagged values as well as the indentity function as the link function. The out-of-sample MSFE for this baseline model on the smaller dataset is \textbf{38.16}, which is better than just using ARIMA on the same dataset.  
...
\subsection{Results}
The MSFE using the VAR model is 76.72 while the MSFE using the VARX model is 75.63. Since the VARX includes exogenous variables and have a better result compared to the VAR model, it shows that the exogenous variables used do have a moderate amount of explanatory power. 

\noindent To improve our forecast accuracy further, the next section will introduce a 3-step modelling approach.

\newpage

\section{3-step Approach}

In this section, we introduce an alternative 3-step approach, which combined unsupervised learning technique, clustering, along with VAR method mentioned above. The overall steps are summarised here:
...
\noindent \textbf{Step 2}. Aggregate the clusters and train a VAR model to predict the total demand for each cluster. 
...
In this 3-step approach, it is not appropriate to use conventional methods, such as the elbow method, to determine the number of clusters. This is because our purpose here is not to minimize the within cluster sum-of-squares, but rather to find the optimal combination of groups that yields the best forecast accuracy. Hence, we tried different number of clusters for the 3 different distance metric as mentioned in Section 7.1 and observe the MSFE for each of those.
...
\\ \hline
\end{tabular}
\textbf{\caption{MSFE for different clustering distance metrics and number of clusters. Bolded results signifying the best result for the respective distance metric.}}
\end{table}

\noindent Based on the results, clustering the time series into 9 groups using the correlation between time series as the distance metric seems to give the best result. 
...
\textbf{\begin{tabular}[c]{@{}l@{}}3-step approach (Using K-means \\ on geographical location)\end{tabular}} & 40.43 & 60.12 &  &  \\ \cline{1-3}
\textbf{\begin{tabular}[c]{@{}l@{}}3-step approach (Using \\ K-means on correlation \\ of time series)\end{tabular}}           & 39.49 & \textbf{59.27}                         &  &  \\ \cline{1-3}
\textbf{\begin{tabular}[c]{@{}l@{}}3-step approach (Using \\ K-means on DTW \\ of time series)\end{tabular}}           & 40.10 & 62.07                         &  &  \\ \cline{1-3}
\end{tabular}
\textbf{\caption{MSFE of the different methods used in this paper}}
\end{table}
