...
\ARTICLEAUTHORS{%
			...
			\AUTHOR{Supervised by: Professor He Long}
	% Enter all authors
}

\ABSTRACT{Many real industry problems involves spatio-temporal demand prediction, which requires predicting demand at a certain time across different locations. Most of the
demand data are not continuous variables but instead count variables, like number
of orders and number of transactions. Previous research have mainly implemented neural
network methods for spatio-temporal problems. In this paper, we would explore more interpretable method and hence will focus on how we can
apply classical time series model, such as Vector Autoregressive (VAR), Spatio-Temporal Autoregressive (STAR) models to exploit spatial
correlations as well as also introduce a 3-step approach to improve forecast accuracy.}
...
\noindent The next section would conduct some exploratory analysis to understand more about the dataset as well as any potential challenges.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth, height=0.45\textheight]{Images/boxplot_counts.jpg}
...
\noindent The out-of-sample MSE for this baseline model on the smaller dataset is \textbf{46.87} and the quadratic score is \textbf{-19.12} while the MSE for dataset of all locations is \textbf{71.73} and the quadratic score is \textbf{-772.45}. A forecast plot on a random location is shown below:
...
\noindent The next section would explore using the Generalized Linear Model (GLM) approach. 


\newpage
\section{GLM Model}
The dataset that we are using follows a count time series, which means the observations are non-negative integers. A flexible and commonly used model for count time series is the Generalized Linear Model (GLM) \cite{Nelder1972}. GLM model makes an assumption that each value is conditionally dependent on its past values. According to \cite{Tobias2017}, an advantage of GLM models over typical ARIMA models is that GLM can describe covariate effects and negative correlations in a more direct way. GLM normally take the form of: \\
...
\noindent As mentioned in section 3.2, we would first assess our model on the smaller dataset before testing it on the full dataset. Using the R package tscount from \cite{Tobias2017}, we fit each of the locations individually by a GLM model. For the parameters, we used the past \textbf{6} lagged values as well as the \textbf{identity function} as the link function, with poisson distribution as the conditional distribution. The out-of-sample MSE for this baseline model on the smaller dataset is \textbf{38.16}, which is better than just using ARIMA on the same dataset. The quadratic score for GLM on the smaller dataset is \textbf{-19.90}. \\
...
\noindent We can observe that there are certain locations' time series which has relatively high correlation with other locations' time series. This thus provides the motivation to use the existence of spatial correlation and explore the usage of VAR model.
...
\textbf{2}                  & 69.71                                              & 62.83 & 62.96                            \\ \hline
\textbf{3}                  & 60.11                                     & 61.97 & 61.49                            \\ \hline
\textbf{4}                  & 62.09                                              & 65.51 & 61.28                            \\ \hline
\textbf{5}                  & 60.28 & 64.31 & 61.00                            \\ \hline
\textbf{6}                  & 63.35 & 63.95 & 61.40                            \\ \hline
\textbf{7}                  & 61.68 & 65.12 & 60.73                            \\ \hline
\textbf{8}                  & \textbf{58.19} & 63.13 & \textbf{60.36}                   \\ \hline
\textbf{9}                  & 63.27 & 64.50 & 60.55                            \\ \hline
\textbf{10}                 & 62.55                                              & \textbf{58.48} & 61.37                            \\ \hline
\textbf{11}                 & 59.53                                              & 64.26 & 63.44                            \\ \hline
\textbf{12}                 & 61.38 & 65.39 & 66.15                            \\ \hline
\end{tabular}
\textbf{\caption{MSE result from using VAR to make cluster predictions for different cluster size and distance metrics}}
\end{table}

\subsection{Quadratic Score from using VAR to make cluster predictions for different cluster size and distance metric}
...
\hline
\textbf{Cluster Size} & \textbf{Euclidean Distance} & \textbf{Correlation between time series} & \textbf{DTW Distance} \\ \hline
\textbf{2}                  & -785.07 & -786.50 & -785.32 \\ \hline
\textbf{3}                  & -786.93 & \textbf{-786.66} & -785.45 \\ \hline
\textbf{4}                  & -786.41 & -786.06 & -785.70                            \\ \hline
\textbf{5}                  & -786.44 & -761.14 & -785.75 \\ \hline
\textbf{6}                  & -776.18 & -741.99 & -785.67 \\ \hline
\textbf{7}                  & -776.14 & -724.84 & \textbf{-785.88} \\ \hline
\textbf{8}                  & \textbf{-787.15} & -694.31 & -785.25 \\ \hline
\textbf{9}                  & -741.26 & -693.53 & -785.45 \\ \hline
\textbf{10}                 & -777.10 & -678.20 & -784.90                            \\ \hline
\textbf{11}                 & -776.79 & -653.50 & -784.61 \\ \hline
\textbf{12}                 & -785.73 & -737.14 & -784.37 \\ \hline
\end{tabular}
\textbf{\caption{Quadratic Score from using VAR to make cluster predictions for different cluster size and distance metric}}
\end{table}

\newpage
\subsection{MSE result from using STAR to make cluster predictions for different cluster size and distance metrics}
...
\hline
\textbf{Cluster Size} & \textbf{Euclidean Distance} & \textbf{Correlation between time series} & \textbf{DTW Distance} \\ \hline
\textbf{2}                  & 61.92 & 68.10 & 61.43 \\ \hline
\textbf{3}                  & 60.91                                     & 64.58 & 60.52                            \\ \hline
\textbf{4}                  & 62.40 & 64.01 & 60.99                            \\ \hline
\textbf{5}                  & 61.14 & \textbf{61.35} & 60.34 \\ \hline
\textbf{6}                  & 60.83 & 64.03 & 61.38 \\ \hline
\textbf{7}                  & \textbf{58.95} & 63.86 & 60.83                            \\ \hline
\textbf{8}                  & 63.03                                              & 66.87 & 59.42                   \\ \hline
\textbf{9}                  & 62.06 & 67.95 & 59.00                            \\ \hline
\textbf{10}                 & 59.08                                              & 61.87 & 58.73                            \\ \hline
\textbf{11}                 & 61.25                                              & 63.85 & \textbf{58.53} \\ \hline
\textbf{12}                 & 60.15 & 64.60 & 59.37 \\ \hline
\end{tabular}
\textbf{\caption{MSE result from using STAR to make cluster predictions for different cluster size and distance metrics}}
\end{table}

\subsection{Quadratic Score from using STAR to make cluster predictions for different cluster size and distance metric}
...
\hline
\textbf{Cluster Size} & \textbf{Euclidean Distance} & \textbf{Correlation between time series} & \textbf{DTW Distance} \\ \hline
\textbf{2}                  & -786.57 & -786.48 & -786.91 \\ \hline
\textbf{3}                  & -753.94 & -786.15 & -786.97 \\ \hline
\textbf{4}                  & -758.61 & -786.65 & -786.97                            \\ \hline
\textbf{5}                  & -757.77 & \textbf{-787.25} & -787.09 \\ \hline
\textbf{6}                  & \textbf{-787.01} & -740.66 & -786.93 \\ \hline
\textbf{7}                  & -760.05 & -755.22 & -787.00 \\ \hline
\textbf{8}                  & -760.44 & -699.55 & -787.09 \\ \hline
\textbf{9}                  & -752.48 & -746.15 & -787.23 \\ \hline
\textbf{10}                 & -732.82 & -728.37 & \textbf{-787.25}                            \\ \hline
\textbf{11}                 & -738.39 & -682.46 & -787.19 \\ \hline
\textbf{12}                 & -741.95 & -647.06 & -787.21 \\ \hline
\end{tabular}
\textbf{\caption{Quadratic Score from using STAR to make cluster predictions for different cluster size and distance metric}}
\end{table}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth, height=0.6\textheight]{Images/Cluster_Combi_MSE_plot.jpg}
    \textbf{\caption{Plot of MSE using the 3-step approach which uses VAR to predict cluster demands. We can see that the optimal choice based on MSE would be using the euclidean distance of the locations with 8 clusters, giving a MSE of 58.19}}
    \label{fig:PPlot of MSE using the 3-step approach which uses VAR to predict cluster demands. We can see that the optimal choice based on MSE would be using the euclidean distance of the locations with 8 clusters, giving a MSE of 58.19}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth, height=0.6\textheight]{Images/Cluster_Combi_quad_plot.jpg}
    \textbf{\caption{Plot of Quadratic score using the 3-step approach which uses VAR to predict cluster demands. We can see that the optimal choice based on quadratic score is using euclidean distance of the locations with 8 custers, giving a quadratic score of -787.15}}
    \label{fig:Plot of Quadratic score using the 3-step approach which uses VAR to predict cluster demands. We can see that the optimal choice based on quadratic score is using euclidean distance of the locations with 8 custers, giving a quadratic score of -787.15}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth, height=0.6\textheight]{Images/Cluster_Combi_STAR_MSE_plot.jpg}
    \textbf{\caption{Plot of MSE using the 3-step approach which uses STAR to predict cluster demands. We can see that the optimal choice based on MSE would be using the DTW between time series with 11 clusters, giving a MSE of 58.53}}
    \label{fig:Plot of MSE using the 3-step approach which uses STAR to predict cluster demands. We can see that the optimal choice based on MSE would be using the DTW between time series with 11 clusters, giving a MSE of 58.53}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth, height=0.6\textheight]{Images/Cluster_Combi_STAR_quad_plot.jpg}
    \textbf{\caption{Plot of Quadratic score using the 3-step approach which uses STAR to predict cluster demands. We can see that the optimal choice based on quadratic score is using DTW between time series with 10 custers, giving a quadratic score of -787.25}}
    \label{fig:Plot of Quadratic score using the 3-step approach which uses STAR to predict cluster demands. We can see that the optimal choice based on quadratic score is using DTW between time series with 10 custers, giving a quadratic score of -787.25}
\end{figure}
...
%\begin{figure}[H]
%    \centering
%    \includegraphics[width=0.95\textwidth, height=0.6\textheight]{Images/Cluster_Loc_quad_plot.jpg}
%    \textbf{\caption{Plot of Quadratic Score against number of clusters. Distance metric used: Euclidean Distance of geographical locations. The optimal number of clusters is 3 with a score of -786.93.}}
%    \label{fig:Plot of Quadratic Score against number of clusters. Distance metric used: Euclidean Distance of geographical locations. The optimal number of clusters is 3 with a score of -786.93.}
%\end{figure}
%
%\begin{figure}[H]
%    \centering
%    \includegraphics[width=0.95\textwidth, height=0.6\textheight]{Images/Cluster_Corr_MSE_plot.jpg}
%    \textbf{\caption{Plot of MSE against number of clusters. Distance metric used: Correlation of time series. The optimal number of clusters is 6 with a MSE of 59.27.}}
%    \label{fig:Plot of MSE against number of clusters. Distance metric used: Correlation of time series. The optimal number of clusters is 6 with a MSE of 59.27.}
%\end{figure}
%
%\begin{figure}[H]
%    \centering
%    \includegraphics[width=0.95\textwidth, height=0.6\textheight]{Images/Cluster_Corr_quad_plot.jpg}
%    \textbf{\caption{Plot of Quadratic Score against number of clusters. Distance metric used: Correlation of time series. The optimal number of clusters is 7 with a score of -787.27.}}
%    \label{fig:Plot of Quadratic Score against number of clusters. Distance metric used: Correlation of time series. The optimal number of clusters is 7 with a score of -787.27.}
%\end{figure}
%
%\begin{figure}[H]
%    \centering
...
%\begin{figure}[H]
%    \centering
%    \includegraphics[width=0.95\textwidth, height=0.6\textheight]{Images/Cluster_DTW_quad_plot.jpg}
%    \textbf{\caption{Plot of Quadratic Score against number of clusters. Distance metric used: DTW of time series. The optimal number of clusters is 7 with a score of -785.88}}
%    \label{fig:Plot of Quadratic Score against number of clusters. Distance metric used: DTW of time series. The optimal number of clusters is 7 with a score of -785.88}
%\end{figure}
...
\textbf{3-step approach (DTW of time series,VAR)}         & 37.96             &       -19.69                  & 60.36            & -785.25                 \\ \hline
\textbf{3-step approach (DTW of time series,STAR)}         & 35.16 &       -19.70                  & 58.53          & -787.19                \\ \hline
\end{tabular}
\textbf{\caption{MSE and Quadratic Score of the different methods used in this paper. The MSE is prioritised here, and hence the quadratic score might not be the optimal one.}}
\end{table}

\noindent While both MSE and Quadratic scores are important metrics to evaluate our models, it is often hard to find a model that has the minimum for both metrics. In this paper, we would prioritise the MSE due to its predictive performane measure. The optimal model would be to use the 3-step approach with the euclidean distance between geographical locations as the distance metric, and using VAR to predict the clusters' total demand. We can also observe that while the Quadratic score for this model is not the minimum, it is the second minimum, with only a very minute difference of 0.1 (-787.15 vs -787.25). \\ 

\noindent Our STAR and VARX models performs better than the GLM models on the full dataset, suggesting that the spatial relationship between locations are useful in forecasting. Also, VARX performs better than VAR model in both datasets, suggesting the exogenous variables have some explanatory power and do improve the forecast accuracy. Applying the 3-step approach also gives a much improved result as compared to just using VAR/VARX or STAR models on all the locations. This might be due to the 3-step approach being more reasonable in dealing with locations with sparse demand due to the aggregation of clusters in the clustering method. In the 3-step approach, our VAR model uses just the 8 clusters as variables to predict the total cluster demand for just 8 clusters, while the original VAR uses all 839 locations as variables to predict for 839 locations, which is more likely to produce a higher error rate.  \\

\noindent As for the distance metric used for clustering, using euclidean distance between the geographical location gives us the best result. However, using correlation or DTW between the time series to cluster also gives similar results and that would imply that spatial correlation might not only be limited to just purely geographical proximity but also on the time series similarity. Comparing correlation measure and DTW measure, using correlation between the time series as the distance metric gives us the better result. Correlation compares the general shape and trend of the time series regardless of scale, whereas for DTW, it is more effective to compare time series of varying length and speed. 
